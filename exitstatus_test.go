...
func TestPaneDoneLocked(t *testing.T) {
	...
	cases := []struct {
		name string
		pane *Pane
		// noIdleDone is --no-idle-done. It withdraws the claim that an idle
		// turn finishes a session; it must not touch any other branch.
		noIdleDone bool
		want       bool
	}{
		{
		...
		{
			// --no-idle-done withdraws the CLAIM. inputReady is still set and
			// still reported; it simply no longer finishes the session, so -w
			// waits for the process to exit.
			name:       "idle, alive, under --no-idle-done",
			pane:       &Pane{inputReady: true, cmd: someCmd()},
			noIdleDone: true,
			want:       false,
		},
		{
			// The flag withdraws idle and nothing else: a reaped process is
			// still done, or --no-idle-done would mean "never done".
			name:       "dead and reaped, under --no-idle-done",
			pane:       &Pane{dead: true, reaped: true, deadAt: time.Now(), cmd: someCmd()},
			noIdleDone: true,
			want:       true,
		},
		{
			// The two features composed: idle no longer wins, and the dead
			// branch still demands proof, so the exit status is still awaited.
			name:       "dead, unreaped, but idle, under --no-idle-done",
			pane:       &Pane{dead: true, deadAt: time.Now(), inputReady: true, cmd: someCmd()},
			noIdleDone: true,
			want:       false,
		},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if got := paneDoneLocked(c.pane, c.noIdleDone); got != c.want {
				t.Fatalf("paneDoneLocked(noIdleDone=%v) = %v, want %v", c.noIdleDone, got, c.want)
			}
		})
	}
}
