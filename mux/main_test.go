...
// avoiding a per-test `go build`. go test runs with cwd = this package's
// directory, so the command package is ../cmd/magmux.
...
func magmuxBinForTest(t *testing.T) string {
	...
	// Fallback: an already-built binary in the repo root (cwd is mux/).
